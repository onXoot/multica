  ...
  test(`Google callback keeps its single-use code while synchronizing ${scenario.name}`, async ({
    ...
    page.on("request", (request) => {
      if (
        ...
        new URL(request.url()).pathname === "/login/callback"
      ) {
        callbackDocuments += 1;
      }
    });
    ...
    try {
      await page.goto(`/login/callback?code=${code}`, { waitUntil: "domcontentloaded" });
      ...
      await expect(page).toHaveURL(`${origin}/login/callback?code=${code}`);
      expect(exchanges).toEqual([{ code, redirect_uri: `${origin}/login/callback` }]);
